...
```bash
sudo docker exec -it psadmin-backend-1 bench --site <server-ip> install-app hrms
```

### 5. Run Migrations
```bash
sudo docker exec -it psadmin-backend-1 bench --site <server-ip> migrate
```
...
**Error:** `Access denied for user '<db-user>'@'<new-container-ip>'`  
**Cause:** DB user locked to old container IP after rebuild  
**Fix:**
```sql
RENAME USER '<db-user>'@'<old-container-ip>' TO '<db-user>'@'%';
GRANT ALL PRIVILEGES ON `<db-user>`.* TO '<db-user>'@'%';
FLUSH PRIVILEGES;
```
...
**Error:** nginx returning 404 for `http://<server-ip>`  
**Cause:** `FRAPPE_SITE_NAME_HEADER` was set to `<your-domain>` but browser sent IP as Host header  
**Fix:**
```bash
cp -r sites/<your-domain> sites/<server-ip>
# Updated compose: FRAPPE_SITE_NAME_HEADER: <server-ip>
```
...
# Backup site
sudo docker exec -it psadmin-backend-1 bench --site <server-ip> backup
...
- [ ] Add GoDaddy DNS A record: `erp` → `<public-ip>`
- [ ] Configure router port forwarding 80 → <server-ip>
